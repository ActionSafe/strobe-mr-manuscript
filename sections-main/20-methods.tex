...
Supplementary materials can have different file types, and support for non-text data by non-multimodal LLMs may be inadequate. To provide appropriate context to the model, we used the following rules to extract texts from them:  
\begin{enumerate}
...
Describe the setting, locations, and relevant dates, including periods of recruitment, exposure, follow-up, and data collection, when available}. A MR article can sometimes only present part of this information, so some information can be lost if we only give a simple "yes" or "no" answer for this item. To avoid losing this information, previous study\cite{gibson_reporting_2023} tried to code this item as "partially" reported. 
